...
from . import importing, matching, reporting, storage
...
def make_server(db_path, web_dir, port):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if url.path == '/api/rematch':
                db = storage.connect(db_path)
                try:
                    payment_id = parse_qs(url.query).get('payment_id', [None])[0]
                    matched = matching.rematch_unmatched(db, payment_id)
                    return self.send(200, {'rematched': matched})
                finally:
                    db.close()
